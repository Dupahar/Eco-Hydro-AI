...
class ModelPredictor:
    ...
    def predict_village(self, graph_path, subsample=None):
        """
        ...
        with torch.no_grad():
            for i in range(0, len(data_sub.x), batch_size):
                ...
                depth, velocity = self.model(batch.x, batch.edge_index, batch.edge_attr)
                all_depths.append(depth.cpu())
                all_velocities.append(velocity.cpu())
        
        # Combine results
        depths = torch.cat(all_depths).numpy().flatten()
        velocities = torch.cat(all_velocities).numpy()
        ...
        if hasattr(data_sub, 'pos'):
            positions = data_sub.pos.cpu().numpy()
        else:
            positions = data_sub.x.cpu().numpy()
...
def main():
    ...
    for i, graph_path in enumerate(graph_files, 1):
        ...
        positions, depths, velocities = predictor.predict_village(
            graph_path,
            subsample=Config.MAX_EXPORT_POINTS
        )
