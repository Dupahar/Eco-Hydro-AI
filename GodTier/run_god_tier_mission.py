...
'''
    ...
    with open("flood_gnn_model.py", "w", encoding="utf-8") as f:
        f.write(code)
    log("✅ Generated Fixed Model: flood_gnn_model.py")
...
'''
    ...
    with open("ingest_data_full_scale.py", "w", encoding="utf-8") as f:
        f.write(code)
    log("✅ Generated Ingest Script: ingest_data_full_scale.py")
...
'''
    ...
    with open("process_graph_full_scale.py", "w", encoding="utf-8") as f:
        f.write(code)
    log("✅ Generated Graph Script: process_graph_full_scale.py")
...
'''
    ...
    with open("train_flood_gnn_full.py", "w", encoding="utf-8") as f:
        f.write(code)
    log("✅ Generated Training Script: train_flood_gnn_full.py")
...
'''
    ...
    with open("export_predictions_full.py", "w", encoding="utf-8") as f:
        f.write(code)
    log("✅ Generated Export Script: export_predictions_full.py")
...
'''
    ...
    with open("vis_cinematic_heatmap.py", "w", encoding="utf-8") as f:
        f.write(code)
    log("✅ Generated Vis Script: vis_cinematic_heatmap.py")
...
def run_step(name, script):
    ...
    if not Path(script).exists():
        log(f"⚠️ Script {script} missing. Skipping.")
        return
    ...
    try:
        ...
        if proc.returncode == 0: log(f"✅ {name} SUCCESS")
        else: log(f"❌ {name} FAILED (Code {proc.returncode})")
...
def main():
    log("🌟 GOD-TIER STATION INITIALIZED 🌟")
